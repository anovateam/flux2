...
func bootstrapGitLabCmdRun(cmd *cobra.Command, args []string) error {
	...
	providerCfg := provider.Config{
		...
		SSHHostname: gitlabArgs.sshHostname,
		Token:       glToken,
	}
	...
	if bootstrapArgs.tokenAuth {
		...
		if gitlabArgs.sshHostname != "" {
			secretOpts.SSHHostname = gitlabArgs.sshHostname
		}
	}
	...
	if gitlabArgs.sshHostname != "" {
		bootstrapOpts = append(bootstrapOpts, bootstrap.WithSSHHostname(gitlabArgs.sshHostname))
	}
	if bootstrapArgs.tokenAuth {
